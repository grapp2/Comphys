...
double integrate(double x1,double y1,double z1,double y11)
{
  ...
  while(x1 < 1.0 - dx/2.0) {
    k1 = z1*dx;
    l1 = (2*y1*y1*y1 + x1*y1)*dx;
    k2 = (z1+0.5*l1)*dx;
    l2 = ((y1+0.5*k1)*(y1+0.5*k1)*(y1+0.5*k1) + x1*(y1+0.5*k1)) * dx;
    k3 = (z1+0.5*l2)*dx;
    l3 = ((y1+0.5*k2)*(y1+0.5*k2)*(y1+0.5*k2) + x1*(y1+0.5*k2)) * dx;
    k4 = (z1+l3)*dx;
    l4 = ((y1+k3)*(y1+k3)*(y1+k3) + x1*(y1+k3))*dx;
